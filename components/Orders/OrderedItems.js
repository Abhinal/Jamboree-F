...
export default function OrderedItems() {
  ...
  useEffect(() => {
    let mounted = true;
    const res = fetch("http://jamboree-b-clone.cloudjiffy.net/api/v1/product/cartDetails", {
      method: "POST",
      headers,
    }).then((res) => {
      if (res.ok) {
    ...
    if (dateForm) {
      if (timeForm) {
        const numRes = await fetch(
          "http://jamboree-b-clone.cloudjiffy.net/api/v1/user/phoneExist",
          {
            method: "GET",
            headers,
          }
          ...
          if (numExist) {
            const response = fetch(
              "http://jamboree-b-clone.cloudjiffy.net/api/v1/product/order",
              {
                method: "POST",
                body: JSON.stringify({
                  sentData,
